...
class LastTransactionsViewModel @Inject constructor(
    ...
    fun getTransactions(user: User) = viewModelScope.launch {
        getTransactionsSafeCall(user)
    }

    private suspend fun getTransactionsSafeCall(user: User) {
        _transactionResponse.value = NetworkResult.Loading()
        if (hasInternetConnection(context)) {
            try {
                val response = repository.getTransactions(user)
